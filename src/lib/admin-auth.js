...
export const login = (username, password) => {
    // Prevent login if credentials are not configured
    if (!ADMIN_CREDENTIALS.username || !ADMIN_CREDENTIALS.password) {
        return { success: false, error: 'Authentication is not configured correctly' };
    }
...
export const changePassword = (oldPassword, newPassword) => {
    // Prevent action if credentials are not configured
