...
export const login = (username, password) => {
    // Secure failure state: If credentials are not configured in the environment, deny all login attempts
    if (!ADMIN_CREDENTIALS.username || !ADMIN_CREDENTIALS.password) {
        console.error('Admin credentials not configured in environment.');
        return { success: false, error: 'System configuration error. Login disabled.' };
    }
